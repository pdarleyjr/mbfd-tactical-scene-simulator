...
export function HomePage() {
  ...
  return <main className="shell min-h-dvh">
    ...
    <section className="mx-auto grid max-w-6xl gap-10 px-5 pb-12 pt-10 lg:grid-cols-[1.15fr_.85fr] lg:items-center lg:pt-20">
      ...
      <div className="panel p-5 md:p-7"><div className="flex items-start justify-between gap-3"><div><p className="eyebrow">Join training</p><h2 className="display mt-2 text-3xl">Choose your room</h2></div><button type="button" className="btn btn-secondary !min-h-11 !px-3" aria-label="Refresh rooms" onClick={() => void refreshRooms()}><RefreshCw size={18}/></button></div><p className="muted mb-5 mt-3 leading-relaxed">Select the named room your instructor opened. A PIN is requested only when that instructor chose to lock the room.</p>
        <form onSubmit={join} className="space-y-4"><label className="block"><span className="mb-2 block font-semibold">Available training rooms</span><select className="field min-h-14" value={roomId} onChange={(event) => setRoomId(event.target.value)} disabled={loadingRooms}><option value="">{loadingRooms ? 'Loading rooms…' : rooms.length ? 'Choose a room' : 'No rooms are open'}</option>{rooms.map((room) => <option key={room.id} value={room.id}>{room.name}{room.locked ? ' · PIN' : ''}{room.currentSession ? ` · ${room.currentSession.scenarioTitle}` : ' · awaiting setup'}</option>)}</select></label>
          {selectedRoom && <div className="border border-[#34434c] bg-[#111a1f] p-3"><strong className="display block">{selectedRoom.name}</strong><span className="muted mt-1 flex items-center gap-2 text-sm">{selectedRoom.locked && <LockKeyhole size={15}/>} {selectedRoom.currentSession ? `${selectedRoom.currentSession.scenarioTitle} · ${selectedRoom.currentSession.status}` : 'Instructor setup is not complete'}</span></div>}
