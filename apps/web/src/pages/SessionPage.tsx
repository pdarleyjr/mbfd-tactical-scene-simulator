...
export function SessionPage() {
  ...
  return <main className="incident-layout">
    <IncidentHeader title={scenario.title} roomName={bootstrap.data.room?.name ?? 'Training room'} unit={actor.unit} name={actor.name} status={collaboration.status} sessionStatus={bootstrap.data.session.status} elapsedMs={bootstrap.data.session.elapsedMs} participantCount={bootstrap.data.participants.length} onVideo={() => ui.setVideoOpen(true)} onActivity={() => void openActivity()} onLeave={leave}/>
    <IncidentRail availableApparatus={scenario.apparatusTemplateIds} availableEvolutions={scenario.evolutionIds} placementTemplateId={ui.placementTemplateId} selectedEvolutionId={activeEvolution?.evolutionId as EvolutionId | undefined} disabled={!canWork || Boolean(auth.permissions?.includes('read-only'))} onTemplateSelect={(id) => ui.setPlacementTemplate(id)} onEvolutionSelect={(id: EvolutionId) => void startEvolution(id)} onTemplateDrop={(id, point) => { if (canvasRef.current?.placeTemplateAtClientPoint(id, point)) placementDone() }}/>
    <section className="incident-stage">
      ...
      {activeEvolution && <div className="active-evolution-card"><div className="flex flex-wrap items-center gap-3"><div className="min-w-0 flex-1"><p className="eyebrow">Active evolution · {actor.unit}</p><strong className="display block text-lg">{activeEvolution.label}</strong><small className="muted">Started at {Math.floor(activeEvolution.startedElapsedMs / 60000).toString().padStart(2, '0')}:{Math.floor(activeEvolution.startedElapsedMs / 1000 % 60).toString().padStart(2, '0')}</small></div><button className="btn btn-primary" onClick={() => void completeEvolution()}>Mark complete</button></div>{taskNotice && <p className="mt-2 text-sm" role="status">{taskNotice}</p>}</div>}
    </section>
