...
import { ArrowLeft, Copy, ImageUp, Play, Plus, RotateCcw, Save, Trash2, Video } from 'lucide-react'
...
export function BuilderPage() {
  ...
  async function remove() { if (!current || !confirm(`Move “${current.title}” to closed scenarios? You can restore it later and existing session history will be preserved.`)) return; try { const next = scenarios.find((scenario) => scenario.id !== current.id); await api(`/api/scenarios/${current.id}`, { method: 'DELETE', token }); await refresh(next?.id ?? null); setMessage('Scenario moved to closed scenarios. It can be restored at any time.'); void navigate(next ? { to: '/builder/$scenarioId', params: { scenarioId: next.id }, replace: true } : { to: '/builder/new', replace: true }) } catch (error) { setMessage(error instanceof Error ? error.message : 'Scenario could not be moved to closed scenarios.') } }
  async function restoreScenario(scenario: ScenarioView) { try { await api(`/api/scenarios/${scenario.id}/restore`, { method: 'POST', token }); await refresh(scenario.id); setShowClosed(false); setMessage(`“${scenario.title}” restored and ready to edit.`); void navigate({ to: '/builder/$scenarioId', params: { scenarioId: scenario.id }, replace: true }) } catch (error) { setMessage(error instanceof Error ? error.message : 'Scenario could not be restored.') } }
