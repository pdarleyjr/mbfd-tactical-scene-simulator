...
export function BuilderPage() {
  ...
  return <main className="shell min-h-dvh"><header className="flex min-h-16 items-center gap-4 border-b border-[#34434c] bg-[#111a1f] px-4"><AppMark/><span className="ml-auto eyebrow">Instructor workspace</span></header>
    <div className="grid min-h-[calc(100dvh-64px)] lg:grid-cols-[290px_minmax(0,1fr)]">
      <aside className="border-r border-[#34434c] bg-[#111a1f] p-4"><p className="eyebrow">Scenario library</p><label className="mt-2 block"><span className="mb-2 block font-semibold">Choose a scenario to edit</span><select aria-label="Scenario library" className="field" value={current?.id ?? ''} onChange={(event) => { const scenario = scenarios.find((item) => item.id === event.target.value); selectScenario(scenario); if (scenario) void navigate({ to: '/builder/$scenarioId', params: { scenarioId: scenario.id } }) }}><option value="">Unsaved new scenario</option>{scenarios.map((scenario) => <option key={scenario.id} value={scenario.id}>{scenario.title}</option>)}</select></label><button className="btn btn-primary mt-4 w-full" onClick={() => { selectScenario(undefined); setRoomName('New Scenario Training Room'); void navigate({ to: '/builder' }) }}><Plus size={18}/>Create new scenario</button><div className="panel mt-4 p-3"><strong className="display block">{current?.title ?? 'New scenario'}</strong><small className="muted mt-1 block">{current ? `Last edited ${new Date(current.updatedAt).toLocaleString()}` : 'Enter a title and save to add it to this menu.'}</small><small className="muted mt-2 block">{scenarios.length} saved scenario{scenarios.length === 1 ? '' : 's'}</small></div></aside>
      <section className="min-w-0 p-4 md:p-6"><form onSubmit={save} className="mx-auto max-w-6xl space-y-5"><div className="flex flex-wrap items-center gap-2"><div className="mr-auto"><p className="eyebrow">Scenario builder</p><h1 className="display text-3xl">{current ? 'Edit scenario' : 'Create scenario'}</h1></div><button type="submit" className="btn btn-primary"><Save size={18}/>Save</button>{current && <><button type="button" className="btn btn-secondary" onClick={() => void duplicate()}><Copy size={18}/>Duplicate</button><button type="button" className="btn btn-danger" onClick={() => void remove()}><Trash2 size={18}/>Delete</button></>}</div>
        ...
        <section className="panel p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="display text-xl">Scenario benchmarks</h2><p className="muted">Common fireground benchmarks are prefilled. Edit, remove, or add any milestone.</p></div><div className="flex flex-wrap gap-2"><button type="button" className="btn btn-secondary" onClick={() => { update('benchmarks', defaultBenchmarkCatalog.map((item) => ({ ...item }))); setBenchmarkIds(defaultBenchmarkCatalog.map((item) => item.id)) }}>Restore defaults</button><button type="button" className="btn btn-secondary" onClick={() => { const benchmark = { id: `benchmark-${crypto.randomUUID().slice(0, 8)}`, label: 'New benchmark', description: '' }; update('benchmarks', [...form.benchmarks, benchmark]); setBenchmarkIds((items) => [...items, benchmark.id]) }}><Plus size={17}/>Add benchmark</button></div></div><div className="mt-3 space-y-3">{form.benchmarks.map((benchmark, index) => <div key={benchmark.id} className="grid gap-2 border border-[#34434c] p-3 md:grid-cols-[1fr_1.4fr_48px]"><input aria-label={`Benchmark ${index + 1} label`} className="field" value={benchmark.label} onChange={(event) => update('benchmarks', form.benchmarks.map((item, itemIndex) => itemIndex === index ? { ...item, label: event.target.value } : item))}/><input aria-label={`Benchmark ${index + 1} description`} className="field" value={benchmark.description} placeholder="Completion criteria" onChange={(event) => update('benchmarks', form.benchmarks.map((item, itemIndex) => itemIndex === index ? { ...item, description: event.target.value } : item))}/><button type="button" aria-label={`Remove benchmark ${index + 1}`} className="btn btn-danger !w-12 !p-0" onClick={() => { update('benchmarks', form.benchmarks.filter((_item, itemIndex) => itemIndex !== index)); setBenchmarkIds((items) => items.filter((id) => id !== benchmark.id)) }}><Trash2 size={17}/></button></div>)}</div></section>
