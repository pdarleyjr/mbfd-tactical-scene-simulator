...
export function JoinPage() {
  ...
  return <main className="shell min-h-dvh"><header className="mx-auto flex max-w-3xl items-center justify-between p-5"><AppMark/><Link to="/" className="btn btn-secondary no-underline">Change room</Link></header><section className="mx-auto max-w-xl px-5 py-10"><div className="panel p-5 md:p-7"><p className="eyebrow">Participant setup</p><h1 className="display mt-2 text-3xl">{room?.name ?? 'Loading room…'}</h1><p className="muted mt-2">{room?.currentSession?.scenarioTitle}</p>
    <form onSubmit={submit} className="mt-7 space-y-4"><label className="block"><span className="mb-2 block font-semibold">Your name</span><input className="field" value={name} onChange={(event) => setName(event.target.value)} required maxLength={80} autoComplete="name"/></label>
