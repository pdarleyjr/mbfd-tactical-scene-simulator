...
export function ContextToolbar(props: { mode: CanvasMode; selected: boolean; canDelete: boolean; hoseEnabled?: boolean; command300: boolean; workspace: 'operations' | '300-plan'; onMode: (mode: CanvasMode) => void; onFit: () => void; onDelete: () => void; onWorkspace: (workspace: 'operations' | '300-plan') => void }) {
  const tool = (mode: CanvasMode, label: string, icon: React.ReactNode, disabled = false) => <button disabled={disabled} aria-label={label} className={`btn !min-h-10 !px-3 ${props.mode === mode ? 'btn-primary' : 'btn-secondary'}`} onClick={() => props.onMode(mode)} aria-pressed={props.mode === mode}>{icon}<span className="hidden lg:inline">{label}</span></button>
  return <nav className="context-bar" aria-label="Map tools">
    {tool('select', 'Select', <MousePointer2 size={17}/>)}
    {tool('pan', 'Pan', <Hand size={17}/>)}
    {tool('hose-attack175', '1¾ attack', <Route size={17} color="#35a854"/>, !props.hoseEnabled)}
    {tool('hose-hose3', '3-inch', <Route size={17} color="#f4f4ee"/>, !props.hoseEnabled)}
    {tool('hose-supply5', '5-inch supply', <Route size={17} color="#f2c230"/>, !props.hoseEnabled)}
