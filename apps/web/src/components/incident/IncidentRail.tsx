...
export function IncidentRail(props: {
  ...
  return <aside className="incident-rail" aria-label="Incident equipment">
    ...
    {evolutionCatalog.filter((item) => props.availableEvolutions.includes(item.id)).map((item) => <button key={item.id} disabled={props.disabled} className="rail-tool" aria-pressed={props.selectedEvolutionId === item.id} title={item.summary} onClick={() => props.onEvolutionSelect(item.id)}>{item.id.includes('lay') ? <Droplets className="mx-auto mb-1 text-[#57a8df]" size={28}/> : <Flame className="mx-auto mb-1 text-[#d9c8a5]" size={28}/>}<strong className="display text-[.68rem]">{item.label}</strong></button>)}
  </aside>
}
